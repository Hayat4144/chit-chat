...
const AddMemebersToGroupChat = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const isMemeberExist = isGroupChat.members.some((value) =>
      members.includes(value.toString()),
    );
    ...
    if (isMemeberExist)
      return next(new CustomError('User already exist in group', 400));
    ...
    const addNewMembers = await groupChatService.addMembers(members, chatId);
    return res.status(httpStatusCode.OK).json({ data: addNewMembers });
  },
);

export default AddMemebersToGroupChat;
